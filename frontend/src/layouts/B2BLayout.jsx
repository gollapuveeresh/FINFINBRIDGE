...
export default function B2BLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-bg">
      {/* ── Sidebar ── */}
      <aside className="fixed left-0 top-0 h-full w-[260px] bg-surface border-r border-border flex flex-col z-50">
        {/* Brand */}
        <Link to="/b2b/dashboard" className="flex items-center gap-3 px-5 py-5 border-b border-border">
      ...
      {/* ── Topbar ── */}
      <header className="fixed top-0 left-[260px] right-0 h-14 bg-surface border-b border-border flex items-center justify-between px-6 z-40">
        <div className="flex items-center gap-2 text-sm text-text-muted">
          <span className="material-symbols-outlined text-base">business</span>
          <span className="font-semibold text-text">{company?.companyName}</span>
          {company?.gstin && <><span className="text-border">·</span><span className="text-xs">{company.gstin}</span></>}
        </div>
      ...
      <main className="ml-[260px] pt-14 min-h-screen">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }} className="max-w-[1400px] mx-auto p-6 space-y-6">
          {children}
        </motion.div>
      </main>
    </div>
