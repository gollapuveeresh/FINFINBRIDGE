...
export default function ReportDetail() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.error('Failed to fetch profile in report details:', err);
    ...
    return (
      <ClientLayout>
        <div className="min-h-[60vh] flex flex-col items-center justify-center">
          ...
          <p className="text-body-md text-text-muted mt-4 font-semibold">Loading report details...</p>
        </div>
      </ClientLayout>
    );
  }
    ...
    return (
      <ClientLayout>
        <div className="min-h-[60vh] flex flex-col items-center justify-center max-w-xl mx-auto text-center space-y-4">
          ...
          <h2 className="text-headline-md font-bold text-accent">Error Loading Report Details</h2>
  ...
  return (
    <ClientLayout>
      <div className="flex items-center gap-2 text-label-lg text-text-muted mb-6">
        <Link to="/client/reports" className="hover:text-secondary transition-colors">Reports</Link>
        <span className="material-symbols-outlined text-sm">chevron_right</span>
        <span className="text-accent font-bold">Report Detail</span>
      </div>
      ...
      <div className="card p-12 text-center flex flex-col items-center justify-center max-w-xl mx-auto space-y-4">
        ...
        <div>
          ...
          <p className="text-body-md text-text-muted mt-2">
            The requested financial performance report is not available for viewing.
          </p>
        </div>
        <Link to="/client/reports">
          <button className="btn-primary mt-2">Back to Reports</button>
        </Link>
      </div>
    </ClientLayout>
  );
